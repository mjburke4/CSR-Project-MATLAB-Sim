function report = run_tranche20_validation(outputRoot,options)
%RUN_TRANCHE20_VALIDATION Full-campus unchanged-default seed comparison.
% Phase: all (default), tests, s129, s130 or finalize. Completed stages can be
...
if nargin<1 || isempty(outputRoot), outputRoot=fullfile(root,'results','t20'); end
if nargin<2, options=struct(); end
options=csr.validation.MultiSeedCheckpoint.options(options);
directory=csr.validation.Artifacts.canonicalPath(outputRoot);
assertOutputPath(root,directory); assertPackagePath(root);
[identity,candidate]=csr.validation.MultiSeedCheckpoint.identity(root);
[items,plan]=csr.scenario.tranche20Suite();
csr.validation.MultiSeedCheckpoint.preflight(directory,root,identity,candidate);
if ~isfolder(directory)
    [ok,message]=mkdir(directory);
    if ~ok, error('csr:t20:Output','Cannot create output: %s.',message); end
end
lockPath=fullfile(directory,'.active'); lock=javaObject('java.io.File',lockPath);
if ~lock.createNewFile()
    error('csr:t20:ActiveRun','Output has an active or interrupted invocation; preserve it and use a new directory.');
end
...
report=struct('Schema','csr-matlab-tranche-20-validation-v1','Tranche',20, ...
    ...
    'CandidateFile','evidence/tranche-20-candidate.json','CandidateSHA256',identity.CandidateSHA256, ...
    ...
    'CommonRandomNumbersClaimed',false,'SingleSeedScope',false,'ComparisonSeeds',[128 129 130], ...
    'ReusedCaseCount',1,'ReusedSeeds',128, ...
    'ReusedBaseline',csr.validation.MultiSeedCheckpoint.reusedBaseline(root,identity.Runtime), ...
    'EvidenceArchive','t20.zip','Artifacts',struct([]),'LocalArtifacts',struct([]), ...
    'InventoryExcludedPaths',{{'metadata.json','.active','t20.zip'}}, ...
    'Scope',['Full portable regression plus two fresh original 6000-second campus runs. ' ...
        'Only seeds 129 and 130 differ from original seed 128; actual-tx stays unchanged. ' ...
        ...
        'Three-seed descriptive comparisons and descriptive 5-percent comparison bands require return review.']);
try
    ...
    writeMetadata();
    fprintf('Tranche 20, phase %s: %d expected portable tests, two 6000-second campus cases.\n', ...
        options.Phase,numel(report.ExpectedTestNames));
    fprintf('Verified all %d T19 sources unchanged; accepted seed 128 is reused.\n', ...
        report.BaselineSourceFilesVerified);
    if ismember(options.Phase,{'all','tests'})
        [receipt,reused]=csr.validation.MultiSeedCheckpoint.begin(directory,'tests',identity);
        if reused
            ...
            summary=csr.validation.MultiSeedCheckpoint.testResults(rows,report.ExpectedTestNames,report.TestFiles);
            ...
            if ~summary.TestsPassed
                error('csr:t20:Tests','Portable regression did not pass; failed evidence is preserved.');
            end
            checkStableInputs();
            receipt=csr.validation.MultiSeedCheckpoint.seal(directory,'tests',identity,summary);
        end
    ...
    for k=1:numel(items)
        ...
        [receipt,reused]=csr.validation.MultiSeedCheckpoint.begin(directory,key,identity);
        if reused
            fprintf('Reusing the verified completed %s campus stage.\n',key);
        else
            summary=csr.validation.multiSeedContract(fullfile(directory,key),key,root, ...
                identity.SourceFiles,report.SourceSnapshotSHA256);
            checkStableInputs();
            receipt=csr.validation.MultiSeedCheckpoint.seal(directory,key,identity,summary);
        end
    ...
    for phase={'tests','s129','s130'}
        ...
        if required || isfile(fullfile(directory,key,'receipt.json'))
            receipt=csr.validation.MultiSeedCheckpoint.verify(directory,key,identity);
    ...
    function writeOnceJson(relative,value)
        ...
        if isfile(path)
            if ~isequal(jsondecode(fileread(path)),value)
                error('csr:t20:OutputIdentity','Existing %s has different inputs.',relative);
            end
        else
    ...
    function copyOnce(relative,target)
        ...
        if isfile(path)
            if ~strcmp(csr.validation.Artifacts.sha256(path),csr.validation.Artifacts.sha256(source))
                error('csr:t20:OutputIdentity','Existing %s differs from the candidate.',target);
            end
        else
            [ok,message]=copyfile(source,path);
            if ~ok, error('csr:t20:Output','Cannot copy frozen input: %s.',message); end
        end
    end
    function checkStableInputs()
        current=csr.validation.MultiSeedCheckpoint.identity(root);
        ...
        if ~isequal(current,identity)
            error('csr:t20:InputChanged','Candidate, source, reference or MATLAB runtime changed during execution.');
        end
    end
    ...
    function attachTests(receipt)
        ...
        computed=csr.validation.MultiSeedCheckpoint.testResults(rows,report.ExpectedTestNames,report.TestFiles);
        ...
        if ~isequal(jsondecode(jsonencode(receipt.summary)),stored) || ...
            ...
            error('csr:t20:Tests','Completed regression does not prove exact passing test membership.');
        end
    ...
    function attachCase(receipt,key)
        ...
        if ~isequaln(jsondecode(jsonencode(receipt.summary)),stored) || ...
                ~strcmp(stored.Schema,'csr-tranche20-multi-seed-summary-v1') || ...
            ...
            error('csr:t20:CampusCompletion','Completed %s receipt has different or incomplete experiment scope.',key);
        end
    ...
    function addReceipt(phase)
        ...
        [~,indices]=ismember({report.StageReceipts.Phase},{'tests','s129','s130'});
        [~,order]=sort(indices); report.StageReceipts=report.StageReceipts(order);
    end
    function finishEvidence()
        files=csr.validation.Artifacts.fileInventory(directory,{'metadata.json','.active','t20.zip'});
        local=endsWith({files.path},'.mat') | endsWith({files.path},'.zip');
        report.Artifacts=files(~local); report.LocalArtifacts=files(local);
        writeMetadata();
        zip(fullfile(directory,'t20.zip'),[{report.Artifacts.path},{'metadata.json'}],directory);
    end
end
...
if strcmp(directory,root) || startsWith(root,[directory filesep])
    error('csr:t20:Output','Output must be separate from the installation and its ancestors.');
end
for name={'+csr','tests','examples','scripts','data','scenarios','evidence'}
    ...
    if strcmp(directory,protected) || startsWith(directory,[protected filesep])
        error('csr:t20:Output','Output cannot overwrite source or reference files.');
    end
end
end
...
names={'csr.sim.NetworkSimulation','csr.sim.EventScheduler','csr.phy.SignalEngine', ...
    ...
    'csr.validation.Artifacts','csr.validation.MultiSeedCheckpoint', ...
    'csr.validation.multiSeedContract','csr.scenario.benchmarkSuite','csr.scenario.tranche20Suite'};
for k=1:numel(names)
    ...
    if isempty(actual) || ~strcmp(expected,csr.validation.Artifacts.canonicalPath(actual))
        error('csr:t20:Path','A different installation is active for %s.',names{k});
    end
end
end
