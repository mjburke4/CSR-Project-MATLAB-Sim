function report = run_tranche14_validation(outputRoot)
%RUN_TRANCHE14_VALIDATION Real ACK service at the DATA-arrival boundary.
% Uses unchanged production layers with prescribed successful transport and
% full-precision timing observations. Upload t14.zip even when traces differ.
root=fileparts(mfilename('fullpath')); addpath(root,'-begin');
if nargin<1 || isempty(outputRoot), outputRoot=fullfile(root,'results','t14'); end
...
if isfolder(directory), error('csr:t14:OutputExists','Output already exists: %s.',directory); end
[ok,message]=mkdir(directory);
if ~ok, error('csr:t14:Output','Cannot create output: %s.',message); end
diary(fullfile(directory,'run.log'));
diaryCleanup=onCleanup(@()diary('off')); %#ok<NASGU>
report=struct('Schema','csr-matlab-tranche-14-validation-v1','Tranche',14, ...
    ...
    'CandidateFile','evidence/tranche-14-candidate.json','CandidateSHA256','', ...
    ...
    'TestResultsFile','tests.csv','BoundaryDirectory','edge', ...
    'BoundaryCompleted',false,'BoundaryPassed',false,'BoundaryMatchesNative',false, ...
    'BoundaryCaseCount',0,'BoundaryEventCount',0, ...
    'BoundaryCheckpointCount',0,'BoundaryUnmatchedCount',0, ...
    ...
    'EvidenceArchive','t14.zip','InventoryExcludedPaths',{{'metadata.json'}}, ...
    'Artifacts',struct([]),'LocalArtifacts',struct([]), ...
    'Scope',['Production receiver MAC/HOP/NWK ACK service around a prescribed mixed ACK/DATA arrival, ' ...
        'with full-precision event observations and before/tie/after insertion controls. ' ...
        'Local transport quantization is confined to a separate diagnostic case. ' ...
        'Sender admission, RF, routing convergence, campus and full-network parity are outside scope.']);
...
try
    assertPackagePath(root,{'csr.mac.Layer','csr.hop.Layer','csr.nwk.Layer', ...
        ...
        'csr.validation.ackEdgeContract', ...
        'csr.validation.ReplayStreams'});
    candidatePath=fullfile(root,report.CandidateFile);
    candidate=jsondecode(fileread(candidatePath));
    if ~strcmp(candidate.Schema,'csr-tranche-14-candidate-v1') || ...
            ~strcmp(candidate.SourceCommit,report.SourceCommit)
        error('csr:t14:CandidateIdentity','Unexpected Tranche 14 candidate or native source pin.');
    end
    ...
    [report.BaselineSourceFilesVerified,report.BaselineMatlabFilesVerified]= ...
        verifyBaseline(root,candidate.BaselineSourceSnapshot,candidate.BaseSourceSnapshotSHA256,271,138);
    ...
    for label={'EdgePlan','EdgeReferenceManifest','NativeBuildManifest'}
        ...
        if ~strcmp(csr.validation.Artifacts.sha256(fullfile(root,candidate.(name))),expected)
            error('csr:t14:InputChanged','Pinned %s differs from candidate identity.',name);
        end
    end
    writeMetadata();
    fprintf('Tranche 14: DATA arrival at an ACK transmission opportunity.\n');
    fprintf('Previous source verified: %d MATLAB files unchanged.\n',report.BaselineMatlabFilesVerified);
    if ~strcmp(report.TestFiles{1},'tests/TestEdgeRecords.m')
        error('csr:t14:TestIdentity','The independent record-table preflight must run first.');
    end
    fprintf('Checking record-table types before the simulation cases.\n');
    ...
    preflightNames=sort(string(report.ExpectedTestNames(startsWith(report.ExpectedTestNames,'TestEdgeRecords/'))));
    actualPreflightNames=sort(string({results.Name}));
    if isempty(results) || ~isequal(actualPreflightNames(:),preflightNames(:))
        error('csr:t14:TestIdentity','Record-table preflight test identities differ from the candidate.');
    end
    assertSuccess(results);
    fprintf('Running six ACK-boundary cases with full-precision timing.\n');
    try
        boundary=csr.validation.ackEdgeContract(fullfile(directory,'edge'));
        report.BoundaryCompleted=logical(boundary.DiagnosticCompleted);
        report.BoundaryPassed=logical(boundary.Passed);
        report.BoundaryMatchesNative=logical(boundary.MatchesNative);
        report.BoundaryCaseCount=boundary.CaseCount;
        report.BoundaryEventCount=boundary.EventCount;
        report.BoundaryCheckpointCount=boundary.CheckpointCount;
        report.BoundaryUnmatchedCount=boundary.UnmatchedCount;
    catch failure
        report.BoundaryFailure=failureRecord(failure);
        fprintf(2,'Boundary diagnostic error: %s\n',failure.message);
    end
    writeMetadata();
    if report.BoundaryCompleted
        fprintf('Boundary diagnostic: %d cases, %d observations; native differences %d.\n', ...
            report.BoundaryCaseCount,report.BoundaryEventCount,report.BoundaryUnmatchedCount);
    else
        fprintf('Boundary diagnostic incomplete; no parity conclusion. Preserve the partial evidence.\n');
    end
    ...
    if ~namesMatch, error('csr:t14:TestIdentity','Executed test identities differ from candidate.'); end
    assertSuccess(results);
    if ~report.BoundaryCompleted || ~report.BoundaryPassed || report.BoundaryCaseCount~=6 || ...
            report.BoundaryEventCount<1 || report.BoundaryCheckpointCount<1
        error('csr:t14:Incomplete','A boundary diagnostic failed its structural checks; preserve t14.zip.');
    end
    checkStableInputs();
    report.FocusedGateExecuted=report.TestsPassed && report.BoundaryCompleted && report.BoundaryPassed && ...
...
fprintf('Tranche 14 focused execution complete: %d/%d MATLAB tests.\n',report.PassedTests,report.TestCount);
    ...
    function checkStableInputs()
        ...
        if ~report.SourceFilesStableDuringRun || ~report.ReferenceFilesStableDuringRun
            error('csr:t14:InputChanged','Source or reference files changed during execution.');
        end
    end
end
...
if ~strcmp(csr.validation.Artifacts.sha256(path),expectedHash)
    error('csr:t14:BaselineIdentity','Baseline snapshot differs: %s.',name);
end
...
for k=1:numel(baseline)
    if ~strcmp(csr.validation.Artifacts.sha256(fullfile(root,baseline(k).path)),baseline(k).sha256)
        error('csr:t14:BaselineChanged','Previous source file differs: %s.',baseline(k).path);
    end
end
sourceCount=numel(baseline); matlabCount=sum(endsWith({baseline.path},'.m'));
if sourceCount~=wantedSource || matlabCount~=wantedMatlab
    error('csr:t14:BaselineIdentity','Unexpected source/MATLAB counts in %s.',name);
end
end
...
for k=1:numel(names)
    ...
    if ~strcmp(expected,resolved)
        error('csr:t14:Path','A different CSR package is active for %s: %s.',names{k},resolved);
    end
end
end
...
for k=1:numel(roots)
    ...
    if isempty(inventory), error('csr:t14:Reference','Empty reference directory: %s.',roots{k}); end
...
for k=1:numel(individual)
    ...
    if numel(info)~=1 || info.isdir, error('csr:t14:Reference','Missing reference file: %s.',path); end
...
if numel(unique({files.path}))~=numel(files)
    error('csr:t14:Reference','Reference inventory declarations overlap.');
end
end
