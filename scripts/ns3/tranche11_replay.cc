// Short matched-contention experiment. Real pinned MAC/HOP; explicitly
// prescribed raw draws and successful addressed delivery. No NWK/RF claim.
#include "ns3/core-module.h"
#include "ns3/csr-net-device.h"
#include "ns3/csr-hop-layer.h"
#include "ns3/tranche11-replay-hooks.h"
#include <array>
#include <filesystem>
#include <fstream>
#include <iomanip>
...
struct Context;
struct Node {
  unsigned id = 0, admitted = 0, delivered = 0, releases = 0, wakes = 0, blocked = 0;
...
struct Context {
  std::string name; std::array<Node, 4> nodes;
...
{
  auto& n = ctx->nodes.at(node); if (n.admitted >= 4) return;
  FrameInfo info; info.source = node; info.app = n.admitted + 1;
  Event("offer", node, 1, info);
  if (!n.hop->CanSendToHop(1)) { ++n.blocked; Event("blocked", node, 1, info); return; }
  auto payload = Create<Packet>(16); payload->AddHeader(CsrNetHeader(node, 1, 0));
  payload->AddPacketTag(CsrDifferentialAppTag(info.app));
  auto pendingBefore = n.hop->GetPendingDataCount(); auto resendBefore = n.hop->GetResendQueueSize();
  n.hop->SendData(1, 0, payload, true);
  if (n.hop->GetPendingDataCount() != pendingBefore + 1 || n.hop->GetResendQueueSize() != resendBefore + 1)
    throw std::runtime_error("CanSend allowed but actual HOP insertion did not succeed");
  ++n.admitted; info.seq = n.admitted; info.rate = 128; info.power = 33;
  Event("admit", node, 1, info);
}
void Node::Release(CsrNodeId source, CsrNodeId destination)
{
  if (source != id || destination != 1) throw std::runtime_error("Wrong released network flow");
  ++releases; FrameInfo info; info.source = source; Event("release", id, 1, info);
}
void Node::Wake() { ++wakes; Event("wake", id, id == 1 ? 0 : 1); if (id != 1) Offer(id); }
void Node::Delivery(Ptr<Packet> packet, CsrNodeId peer)
{
  CsrNetHeader net; CsrDifferentialAppTag tag;
  if (!packet->PeekHeader(net) || !packet->PeekPacketTag(tag) || net.GetDst() != id)
    throw std::runtime_error("Invalid controlled application delivery");
  if (!received.insert({net.GetSrc(), tag.GetSequence()}).second)
    throw std::runtime_error("Duplicate application delivery escaped HOP filter");
  ++delivered;
}
               ...
               Time duration, int rate, double power, int preamble, int slot)
{
  for (const auto& frame : frames) {
    ...
    if (receiver < 1 || receiver > 3 || receiver == sender)
...
void RunCase(const Row& input, const std::vector<Row>& tapes, const std::filesystem::path& output)
{
  ...
  ctx->events << "case,order,time_ns,event,node,peer,app_source,app_id,hop_seq,ack_bits,dack_bits,mac_state,ack_queue,data_queue,prep,counter,opportunity,advertised,hop_pending,neighbor_outstanding,neighbor_threshold,resend_queue,admitted,rate_kbps,power_dbm\n";
  ...
  for (unsigned node = 1; node <= 3; ++node)
    if (ctx->tapes[node].size() != 128) throw std::runtime_error("Expected complete 128-entry node tape");
  RngSeedManager::SetSeed(129); RngSeedManager::SetRun(1);
  OpenDifferentialTraceCsv((folder / "native.csv").string());
  for (unsigned node = 1; node <= 3; ++node) {
    ...
    n.hop->ConfigureLinkControl(128, 128, 33, 33, 6);
    n.hop->SetRxFromHopCallback(MakeCallback(&Node::Delivery, &n));
    n.hop->SetNsdpDecrementCallback(MakeCallback(&Node::Release, &n));
    n.hop->SetNwkQueueWakeCallback(MakeCallback(&Node::Wake, &n));
    ...
    for (unsigned peer = 1; peer <= 3; ++peer) if (peer != node && (node == 1 || peer == 1)) {
      mac.NoteHeardFrom(peer, 0); mac.NoteHeardFrom(peer, 0); mac.SetNeighborPathloss(peer, 70);
    }
  }
  csr_t11::rawDraw = RawDraw; csr_t11::resolvedDraw = ResolvedDraw; csr_t11::transport = Transport;
  const double duration = std::stod(input.at("duration_seconds"));
  const double start = std::stod(input.at("track_start_seconds"));
  if (start >= 0) {
    Simulator::Schedule(Seconds(start), [] { ctx->nodes[1].device->GetMac().SetReceiveState(CsrMacCore::State::TRACK); Event("busy", 1, 0); });
    Simulator::Schedule(Seconds(std::stod(input.at("track_stop_seconds"))), [] { ctx->nodes[1].device->GetMac().SetReceiveState(CsrMacCore::State::SEARCH); Event("busy", 1, 0); });
  }
  for (unsigned poll = 0; poll < 400; ++poll)
    Simulator::Schedule(MilliSeconds(poll * 20), [] { Offer(2); Offer(3); });
  Simulator::Stop(Seconds(duration)); Simulator::Run();
  for (unsigned node = 1; node <= 3; ++node) Event("final", node, node == 1 ? 0 : 1);
  ...
  summary << "case,node,admitted,delivered,releases,wakes,blocked,pending,resend_queue,ack_queue,data_queue,draws_consumed,draws_unused\n";
  for (unsigned node = 1; node <= 3; ++node) {
    auto& n = ctx->nodes[node]; auto& mac = n.device->GetMac(); unsigned used = ctx->consumed[node];
    summary << ctx->name << ',' << node << ',' << n.admitted << ',' << n.delivered << ',' << n.releases << ',' << n.wakes << ',' << n.blocked << ',' << n.hop->GetPendingDataCount() << ',' << n.hop->GetResendQueueSize() << ',' << mac.GetAckQueuedFrameCount() << ',' << mac.GetDataQueuedFrameCount() << ',' << used << ',' << 128 - used << '\n';
  }
  CloseDifferentialTraceCsv(); Simulator::Destroy();
  csr_t11::rawDraw = {}; csr_t11::resolvedDraw = {}; csr_t11::transport = {};
  ctx = nullptr;
}
void SelfTest()
{
...
int main(int argc, char** argv)
{
  try {
    ...
    if (argc != 4) { std::cerr << "usage: tranche11_replay cases.csv draws.csv output\n"; return 2; }
