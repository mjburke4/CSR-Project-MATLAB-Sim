...
import subprocess,os,json,gzip,csv,itertools,time
R=Path(__file__).resolve().parents[2];N=R/'mac_replay/native';B=R/'startup131/environment/engine/build';D=N/'capture';D.mkdir(exist_ok=True)
...
assert p.returncode==0,(D/'compile.log').read_text()[-10000:]
A=R/'original_extracted/t25up/evidence/tranche-25-ns3-reference/s132';cmd=[str(N/'capture-exe'),'--scenario='+str(A/'scenario.csv'),'--trace='+str(D/'ns3-trace.csv'),'--appDiagnostics='+str(D/'app-admission.csv'),'--stop=665','--flowLimit=0','--dutyCycling=1','--opnetAlignedDutyCycle=1','--gatewayDiscovery=1','--opnetAppGating=1','--aggregateTraceOnly=0','--admissionTrace=1','--quietModelLogs=0','--stochasticSyncThreshold=1']
env=dict(os.environ,CSR_MAC_CAPTURE=str(D/'mac-input.log'));start=time.monotonic()
...
with gzip.open(A/'ns3-trace.csv.gz','rt') as f,(D/'ns3-trace.csv').open() as g:
 ...
 for x,y in itertools.zip_longest(itertools.takewhile(lambda r:float(r['time_s'])<665,rr),itertools.takewhile(lambda r:float(r['time_s'])<665,ar)):
