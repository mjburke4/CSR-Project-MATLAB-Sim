...
// Timestamp existing stdout without scheduling simulator events.
class StartupTimedBuffer : public std::streambuf {
  std::streambuf* sink; std::string line;
  void emit() {
    std::ostringstream prefix;
    prefix << "time_ns=" << Simulator::Now().GetNanoSeconds() << " ";
    auto value=prefix.str()+line; sink->sputn(value.data(),value.size()); line.clear();
  }
public:
  explicit StartupTimedBuffer(std::streambuf* target):sink(target){}
  int overflow(int c) override { if(c!=traits_type::eof()){line.push_back(char(c));if(c=='\n')emit();}return traits_type::not_eof(c); }
  int sync() override { if(!line.empty())emit();return sink->pubsync(); }
};
...
main (int argc, char *argv[])
{
  ...
  StartupTimedBuffer startupTimed(std::cout.rdbuf());
  auto startupOriginal=std::cout.rdbuf(&startupTimed);
  ...
  std::cout.flush();
  std::cout.rdbuf(startupOriginal);
