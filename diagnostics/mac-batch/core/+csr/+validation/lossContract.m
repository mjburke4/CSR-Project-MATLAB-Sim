function report = lossContract(outputDirectory)
%LOSSCONTRACT Real NWK/HOP/MAC relay service with controlled raw draws.
...
transport = table(); terminals = table();
caseResults = repmat(emptyResult(),0,1);
for k=1:height(cases)
    fprintf('Loss diagnostic %d/%d: %s (%g simulated seconds).\n', ...
        k,height(cases),char(cases.('case')(k)),cases.duration_seconds(k));
    [caseEvents,caseDraws,caseUsage,caseChecks,caseTransport,caseTerminals,result] = runCase(cases(k,:));
...
if ~isempty(outputDirectory)
    ...
    writetable(transport,fullfile(outputDirectory,'transport.csv'));
    writetable(terminals,fullfile(outputDirectory,'terminal.csv'));
...
report.Transport = transport; report.Terminals = terminals;
    ...
    function [outEvents,outDraws,outUsage,outChecks,outTransport,outTerminals,result] = runCase(entry)
        ...
        outTransport = struct2table(transportRows); outTerminals = struct2table(terminalRows);
...
exact = intersect({'app_id','ack_bits','dack_bits'},options.VariableNames,'stable');
