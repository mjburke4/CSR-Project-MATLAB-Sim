#!/usr/bin/env python3
"""Make two explicit test seams in a copied, pinned CSR header tree.
...
def build(source: Path, output: Path):
    ...
    replacement = """        // collision slot R cannot be revisited.
        // TRANCHE11 TEST SEAM: only the raw integer, before native probing.
        int chosenSlot = csr_t11::rawDraw
          ? csr_t11::rawDraw (m_nodeId, 0, slotRange)
    ...
    replacement = """                          << " using historical next_tslot modulo probe"
                          << std::endl;
                if (csr_t11::resolvedDraw)
                  csr_t11::resolvedDraw (m_nodeId, chosenSlot, probes);
                return chosenSlot;"""
    text = once(text, anchor, replacement)
  ...
  // TRANCHE11 TEST SEAM: actual airtime and TX state, prescribed delivery.
  if (csr_t11::transport)
    {
      csr_t11::transport (m_id, frameCopies, Seconds (duration), rateKbps,
  ...
  for (const auto &peer : m_peers)"""
    ...
    if sorted(modified) != ["csr-mac-core.h", "csr-net-device.h"]:
    ...
    manifest = {
        "schema": "csr-tranche11-overlay-v1",
        "scope": "Pinned MAC/HOP with test-only raw draw and controlled transport seams; no RF claim",
