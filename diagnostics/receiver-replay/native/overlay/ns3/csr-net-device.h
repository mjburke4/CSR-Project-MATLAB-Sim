  ...
  bool ackable)
{
  ...
  Rpl::OwnTx(m_id, signalId, txId, sequence, payloadBytes, packetBits, rateKbps, txPowerDbm, preamble, slot, ackable, txTime, duration, preambleSec, frameCopies);
...
CsrNetDevice::CloseSignalInterval (RxSignal &signal, double endSec)
{
  ...
  Rpl::Context(m_id, signal.id, signal.intervalStartSec, boundedEndSec);
...
CsrNetDevice::BeginReceiveSignal (RxSignal signal)
{
  ...
  if (incoming.frontEnd.channelMatched)
    {
      double syncThresholdDb = Rpl::IsReplay(m_id) ? Rpl::SyncValue(incoming.id) : DrawSyncSnrThresholdDb ();
...
CsrNetDevice::AcquireSignal ()
{
  ...
  Rpl::Event(m_id, selected->id, "track", selected->txId, selected->sequence, "", "", int(m_mac.GetState()), 2);
                                    ...
                                    const CsrRxDecision &decision)
{
  ...
  Rpl::Event(m_id, signal.id, "mac_receive", signal.txId, signal.sequence, "accepted", "accepted", int(m_mac.GetState()), int(m_mac.GetState()));
  ...
  for (const auto &segment : signal.frames)
    {
      CsrHeader replayHeader; segment->PeekHeader(replayHeader);
  ...
  if (wasTracked)
    {
      ...
      Rpl::Event(m_id, signal.id, "signal_end", signal.txId, signal.sequence, decision.success ? "accepted" : "dropped", rxEvent.reason, int(m_mac.GetState()), int(m_mac.GetState()));
  ...
  else
    {
      ...
      missEvent.detail = "collisions=" + CsrTraceInteger (
        signal.collisionCount);
      Rpl::Event(m_id, signal.id, "signal_end", signal.txId, signal.sequence, "dropped", missEvent.reason, int(m_mac.GetState()), int(m_mac.GetState()));
                                   ...
                                   bool cancelPostTxWait)
{
  ...
  m_txPreparationActive = true; Rpl::PrepChanged(m_nodeId, true);
...
CsrMacCore::DoTx ()
{
  if (!HasPendingFrame () || m_dev == nullptr)
    {
      m_txPreparationActive = false; Rpl::PrepChanged(m_nodeId, false);
      return;
    }
