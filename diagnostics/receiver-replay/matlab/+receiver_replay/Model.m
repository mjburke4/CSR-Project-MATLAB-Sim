% TEST-ONLY COPY. Generated by generate_copies.py; accepted core is unchanged.
classdef Model
    ...
    methods (Static)
        function result = frontEnd(txProfile, rxProfile, txPosition, rxPosition)
            ...
            distance = norm(double(txPosition(:)) - double(rxPosition(:))) * ...
                rx.DistanceScale;
            receiver_replay.Model.nonnegative(distance, 'distanceMeters');
            ...
            noise = receiver_replay.Model.dbmToWatts(rx.NoiseFloorDbm);
            if rx.ScaleNoiseWithBandwidth
                noise = noise * rx.RxBwHz / rx.NoiseReferenceBwHz;
            end
            [gain, pathloss, pathModel] = receiver_replay.Model.pathGain(tx, rx, distance);
            inBand = receiver_replay.Model.dbmToWatts(tx.TxPowerDbm) * overlap / tx.TxBwHz;
            power = 0;
            if overlap > 0 && gain > 0
                power = inBand * 10^(tx.TxAntennaGainDb / 10) * ...
                    gain * 10^(rx.RxAntennaGainDb / 10);
            end
            receiver_replay.Model.nonnegative(power, 'receivedPowerWatts');
            receiver_replay.Model.nonnegative(noise, 'backgroundNoiseWatts');
            result = struct('Closure', receiver_replay.Model.hasClosure(tx, rx, distance), ...
                ...
                'ReceivedPowerDbm', receiver_replay.Model.wattsToDbm(power), ...
                'BackgroundNoiseWatts', noise, 'NoisePowerWatts', noise, ...
                'NoisePowerDbm', receiver_replay.Model.wattsToDbm(noise), ...
                'SnrDb', receiver_replay.Model.snrDb(power, noise));
        end

        function interval = interval(startSec, endSec, noisePowerWatts)
            receiver_replay.Model.nonnegative(startSec, 'startSec');
            receiver_replay.Model.nonnegative(endSec, 'endSec');
            receiver_replay.Model.nonnegative(noisePowerWatts, 'noisePowerWatts');
            if endSec < startSec
        ...
        function result = ber(rxProfile, frontEnd, rateKbps, interval)
            rx = csr.phy.RadioProfile.validate(rxProfile);
            receiver_replay.Model.validateInterval(interval);
            ...
            snr = receiver_replay.Model.snrDb(frontEnd.ReceivedPowerWatts, ...
            ...
            if highRate && interval.HighRatePayloadJammer
                ...
                payloadSnr = receiver_replay.Model.snrDb(frontEnd.ReceivedPowerWatts, ...
        ...
        function result = allocateErrors(rxProfile, frontEnd, signalStartSec, ...
            ...
            rx = csr.phy.RadioProfile.validate(rxProfile);
            receiver_replay.Model.nonnegative(signalStartSec, 'signalStartSec');
            signalStartSec = double(signalStartSec);
            receiver_replay.Model.bitCount(preambleBits, 'preambleBits');
            receiver_replay.Model.bitCount(packetBits, 'packetBits');
            ...
            for k = 1:numel(intervals)
                receiver_replay.Model.validateInterval(intervals(k));
            end
            ...
            for index = order.'
                ...
                values = receiver_replay.Model.ber(rx, frontEnd, rateKbps, current);
        ...
        function errors = sampleSourceBinomial(bits, probability, uniform)
            ...
            receiver_replay.Model.bitCount(bits, 'bits');
        ...
        function result = ecc(rxProfile, packetBits, preambleBits, totalErrors, ...
            ...
            receiver_replay.Model.bitCount(packetBits, 'packetBits');
            receiver_replay.Model.bitCount(preambleBits, 'preambleBits');
            receiver_replay.Model.bitCount(totalErrors, 'totalErrors');
        ...
        function watts = dbmToWatts(dbm)
            ...
            receiver_replay.Model.nonnegative(watts, 'convertedPowerWatts');
        end

        function dbm = wattsToDbm(watts)
            receiver_replay.Model.nonnegative(watts, 'watts');
            dbm = -Inf;
            if watts > 0
        ...
        function snr = snrDb(signalWatts, noiseWatts)
            receiver_replay.Model.nonnegative(signalWatts, 'signalWatts');
            receiver_replay.Model.nonnegative(noiseWatts, 'noiseWatts');
            snr = -Inf;
    ...
    methods (Static, Access = private)
        ...
        function [gain, loss, name] = pathGain(tx, rx, distance)
            ...
            elseif strcmp(rx.PropagationModel, 'LOG_DISTANCE')
                ...
                gain = 10^(-loss / 10);
                receiver_replay.Model.nonnegative(gain, 'pathGainLinear');
                return
            end
            centerFrequency = tx.TxBaseFrequencyHz + tx.TxBwHz / 2;
            wavelength = receiver_replay.Model.SpeedOfLightMetersPerSecond / centerFrequency;
        ...
        function errors = drawBinomial(bits, probability, stream)
            ...
            elseif probability >= 1
                ...
                errors = receiver_replay.Model.sampleSourceBinomial(bits, probability, uniform);
            end
        end
        ...
        function validateInterval(interval)
            ...
            receiver_replay.Model.nonnegative(interval.StartSec, 'interval.StartSec');
            receiver_replay.Model.nonnegative(interval.EndSec, 'interval.EndSec');
            receiver_replay.Model.nonnegative(interval.NoisePowerWatts, 'interval.NoisePowerWatts');
            receiver_replay.Model.bitCount(interval.CollisionCount, 'interval.CollisionCount');
