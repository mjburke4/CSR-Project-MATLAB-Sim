function [cases,plan] = tranche20Suite()
%TRANCHE20SUITE Two full original campus runs differing only in random seed.
root=fileparts(fileparts(fileparts(mfilename('fullpath'))));
plan=jsondecode(fileread(fullfile(root,'scenarios','t20','plan.json')));
[original,~]=csr.scenario.benchmarkSuite(struct('Cases',{{'campus_multihop_6000'}}));
if numel(original)~=1
    error('csr:t20:Plan','Exactly one original campus benchmark is required.');
end
csr.validation.MultiSeedCheckpoint.validatePlan(plan,original);
template=original;
...
for k=1:2
    ...
    if ~isequaln(cases(k).Config,expected)
        error('csr:t20:Plan','Validation altered settings outside the declared random seed.');
    end
end
end
